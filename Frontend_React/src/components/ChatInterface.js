      ...
      >
        {messages.map((message, index) => (
          ...
          >
            ...
            <Box sx={{ flexGrow: 1, color: '#ECECF1', lineHeight: 1.75, fontSize: '1rem' }}> {/* Apply common text styles here */}
              {/* --- FIX: Use ReactMarkdown for assistant messages --- */}
              {message.role === 'assistant' ? (
                <ReactMarkdown
                  // Apply styles directly or via CSS for elements like p, strong, em, etc.
                  // You might need to add components prop for custom rendering if needed
                  components={{
                    // Example: Style paragraphs generated by markdown
                    p: ({node, ...props}) => <Typography variant="body1" sx={{ mb: 1 }} {...props} />,
                    // Add other elements like h1, h2, ul, ol, li, strong, em as needed
                  }}
                >
                  {message.content}
                </ReactMarkdown>
              ) : (
                // Keep Typography for user messages (or use ReactMarkdown if users can input markdown)
